...
class BacktestEngine:
    ...
    def load_data(self) -> bool:
        try:
            log.info(
                f"Loading data for {self.config.symbol1} and {self.config.symbol2}"
            )
            ...
            if self.aligned_data is None or len(self.aligned_data) < 100:
                log.error(
                    f"Insufficient data: {0 if self.aligned_data is None else len(self.aligned_data)} rows"
                )
                return False

            log.info(f"Data loaded: {len(self.aligned_data)} aligned observations")
            log.info(
                f"Date range: {self.aligned_data.index[0]} to {self.aligned_data.index[-1]}"
            )
            return True
    ...
    def run(self) -> BacktestResults:
        ...
        log.info(
            f"Orderbook mode: {'ENABLED' if self.config.use_orderbook else 'DISABLED'}"
        )
        ...
        for i, (timestamp, row) in enumerate(self.aligned_data.iterrows()):
            ...
            if signal:
                ...
                if (
                    signal["action"] != last_signal_action
        ...
        self._close_all_positions(
            self.aligned_data.index[-1], self.aligned_data.iloc[-1]
        )

        self.results.equity_curve = pd.DataFrame(equity_curve).set_index("timestamp")[
            "equity"
        ]
        self.results.positions = (
            pd.DataFrame(position_history) if position_history else pd.DataFrame()
        )
    ...
    def _close_all_positions(self, timestamp: datetime, row: pd.Series):
        ...
        for position_id in list(self.portfolio.positions.keys()):
            ...
            duration = timestamp - position.timestamp
            log.info(
                f"🔚 FINAL CLOSE | ID:{position_id} | PnL:${pnl:.2f} | Duration:{duration}"
            )
        ...
        if self.slippage_tracking:
            ...
            if self.results.trades:
                t = pd.DataFrame(
                    [
                        x
                        for x in self.results.trades
                        if x.get("action") in ["LONG", "SHORT"]
                    ]
                )
                if not t.empty and "avg_slippage_bps" in t.columns:
                    total_value = t["cost"].sum()
                    avg_slip = t["avg_slippage_bps"].mean()
                    self.results.slippage_analysis["total_slippage_cost"] = (
                        total_value * (avg_slip / 10000)
                    )
    ...
    def _print_enhanced_summary(self):
        ...
        trades_df = (
            pd.DataFrame(self.results.trades) if self.results.trades else pd.DataFrame()
        )
        ...
        log.info(
            f"  Total Return: {m.get('total_return', 0):.2%} | Annual: {m.get('annual_return', 0):.2%}"
        )
        log.info(
            f"  Sharpe: {m.get('sharpe_ratio', 0):.2f} | Calmar: {m.get('calmar_ratio', 0):.2f} | Max DD: {m.get('max_drawdown', 0):.2%}"
        )

        # Trading Statistics Detalladas
        if not trades_df.empty:
            close_trades = trades_df[
                trades_df["action"].isin(["CLOSE", "CLOSE_FINAL"])
            ].copy()

            if not close_trades.empty:
                # Análisis por razón de cierre
                exit_reasons = (
                    close_trades["reason"].value_counts()
                    if "reason" in close_trades.columns
                    else {}
                )
                stop_losses = exit_reasons.get("stop_loss", 0)
                normal_exits = exit_reasons.get("exit_signal", 0) + exit_reasons.get(
                    "final_close", 0
                )
                ...
                log.info(
                    f"  Total Trades: {len(close_trades)} | Wins: {len(wins)} ({len(wins)/len(close_trades)*100:.1f}%) | Losses: {len(losses)} ({len(losses)/len(close_trades)*100:.1f}%)"
                )
                log.info(
                    f"  Normal Exits: {normal_exits} | Stop Losses: {stop_losses} | Stop Loss Rate: {stop_losses/len(close_trades)*100:.1f}%"
                )
                log.info(
                    f"  Avg Win: ${wins.mean():.2f} | Avg Loss: ${losses.mean():.2f} | Win/Loss Ratio: {abs(wins.mean()/losses.mean()) if len(losses) > 0 else float('inf'):.2f}"
                )
                log.info(
                    f"  Best Trade: ${pnls.max():.2f} | Worst Trade: ${pnls.min():.2f} | Profit Factor: {m.get('profit_factor', 0):.2f}"
                )
                log.info(
                    f"  Expectancy: ${m.get('expectancy', 0):.2f} | Avg Trade: ${pnls.mean():.2f} | Median Trade: ${pnls.median():.2f}"
                )

                log.info(f"\n⏱️ TRADE DURATION ANALYSIS:")
                log.info(
                    f"  Avg Duration: {avg_duration_hours:.1f}h | Median: {median_duration_hours:.1f}h | Max: {max_duration_hours:.1f}h"
                )
                ...
                if "z_score" in close_trades.columns:
                    ...
                    log.info(
                        f"  Avg |Z|: {z_scores.mean():.2f} | Median |Z|: {z_scores.median():.2f} | Max |Z|: {z_scores.max():.2f}"
                    )
                    log.info(
                        f"  Extreme Closes (|Z|>3): {(z_scores > 3).sum()} ({(z_scores > 3).mean()*100:.1f}%)"
                    )

        # Strategy Analysis
        strategy_metrics = (
            self.strategy.get_strategy_metrics()
        ...
        if strategy_metrics:
            log.info(f"\n🎯 STRATEGY ANALYSIS:")
            log.info(
                f"  Current Hedge Ratio: {strategy_metrics.get('current_hedge_ratio', 1.0):.4f}"
            )
            log.info(
                f"  Z-Score Range: {strategy_metrics.get('min_z_score', 0):.2f} to {strategy_metrics.get('max_z_score', 0):.2f}"
            )
            log.info(
                f"  Avg Z-Score: {strategy_metrics.get('avg_z_score', 0):.3f} | Std: {strategy_metrics.get('z_score_std', 0):.3f}"
            )
            if "avg_correlation" in strategy_metrics:
                log.info(
                    f"  Avg Correlation: {strategy_metrics.get('avg_correlation', 0):.3f}"
                )
            if "extreme_z_scores" in strategy_metrics:
                log.info(
                    f"  Extreme Signals (|Z|>3): {strategy_metrics.get('extreme_z_scores', 0)}"
                )
            ...
            if "resample_minutes" in strategy_metrics:
                log.info(
                    f"  Resample Minutes: {strategy_metrics.get('resample_minutes', 0)}"
                )
                log.info(
                    f"  Lookback Days: {strategy_metrics.get('lookback_days', 0)} = {strategy_metrics.get('lookback_periods', 0)} periods"
                )
        ...
        if self.results.slippage_analysis:
            ...
            log.info(f"\n💹 EXECUTION ANALYSIS:")
            log.info(
                f"  Avg Slippage: {s.get('avg_slippage_bps', 0):.1f} bps | Max: {s.get('max_slippage_bps', 0):.1f} bps"
            )
            log.info(
                f"  Orderbook Usage: {s.get('orderbook_usage', 0):.1f}% | Slippage Events: {s.get('total_slippage_events', 0)}"
            )
        ...
        log.info(f"\n💵 CAPITAL EFFICIENCY:")
        log.info(
            f"  Initial Capital: ${self.config.initial_capital:,.2f} | Final Equity: ${m.get('final_equity', 0):,.2f}"
        )
        log.info(
            f"  Total PnL: ${m.get('total_pnl', 0):,.2f} | Max Capital Used: {self.config.position_size:.1%}"
        )

        if not trades_df.empty:
            total_volume = (
                trades_df[trades_df["action"].isin(["LONG", "SHORT"])]["cost"].sum()
                if "cost" in trades_df.columns
                else 0
            )
            log.info(
                f"  Total Volume Traded: ${total_volume:,.2f} | Turnover: {total_volume/self.config.initial_capital:.1f}x"
            )
        ...
        if len(self.results.equity_curve) > 0:
            ...
            if len(returns) > 0:
                volatility = returns.std() * np.sqrt(
                    252 * 24 * 60 / self.config.resample_minutes
                )  # Annualized
                var_95 = returns.quantile(0.05)
                log.info(f"\n📉 RISK METRICS:")
                log.info(
                    f"  Annualized Volatility: {volatility:.2%} | VaR(95%): {var_95:.2%}"
                )
                log.info(
                    f"  Best Day: +{returns.max():.2%} | Worst Day: {returns.min():.2%}"
                )
